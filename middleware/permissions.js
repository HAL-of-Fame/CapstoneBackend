  ...
  try {
    ...
    if (post.userEmail !== user.email) {
      throw new ForbiddenError(
        `${user.username} is not allowed to update ${post.userName}'s posts`
      );
    }
  ...
  try {
    ...
    if (post.userEmail == user.email) {
      throw new BadRequestError(
        `Users are not allowed to rate their own posts`
      );
    }
    res.locals.post = post;
    return next();
...
const authUserOwnsComment = async (req, res, next) => {
  try {
    ...
    if (comment.userEmail !== user.email) {
      throw new ForbiddenError(
        `${user.username} is not allowed to update ${comment.userName}'s posts`
      );
    }
